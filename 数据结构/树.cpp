...
//树的存储结构
//抽象类型定义 : 孩子兄弟表示法(二叉树表示法), 其他方法不做介绍
//由此可看出, 树和二叉树是可以互相转换的
struct CSNode {
    type data;
    //左结点存储第一个孩子, 右结点存储下一个兄弟
...
/**
 * 1. 结点的路径长度 : 两结点间的路径长度
 * 2. 树的路径长度 : 从根到每个结点的路径长度之和
 * 3. 权 : 给结点赋有意义的值, 叫做结点的权
 * 4. 结点的带权路径长度 : 根到结点的路径长度 * 结点的权
 * 5. 树的带权路径长度 : 所有叶子结点的带权路径长度之和
 * 
 * 最优树 : 带权路径长度最短的树 (度相同才进行比较)
 * 
 * 哈夫曼树构造方式 : 
 * 1. 将每个权值造成一个树
 * 2. 选择两个权值最小的树合成一个树
